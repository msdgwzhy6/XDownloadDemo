...
public class WaveDrawable extends Drawable implements Animatable, ValueAnimator.AnimatorUpdateListener {
    private static final float WAVE_HEIGHT_FACTOR = 0.1f;
    ...
    //2017-05-14 14:51:24  修订：小民
    public WaveDrawable(Context context, @DrawableRes int imgRes, int width, int height) {
        Drawable drawable;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            drawable = context.getDrawable(imgRes);
        } else {
